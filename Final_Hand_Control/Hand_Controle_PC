...
def control_with_hand():
    ...
    while True:
        ...
        if result.multi_hand_landmarks:
            ...
            if len(lmList):
                finger_count = count_finger(lmList)
                cv2.putText(img, f"Fingers -> {finger_count}", (20, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)

                current_time = time.time()
                if finger_count == 5 and (current_time - last_switch_time) > switch_cooldown:
                ...
                handRange = [50, 300]
                raw_vol = np.interp(length, handRange, [min_vol, max_vol])

                smoothing = 5
                vol = prev_vol + (raw_vol - prev_vol) / smoothing
                prev_vol = vol
