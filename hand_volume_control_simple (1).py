...
# بازه‌ی واقعی دسی‌بل صدای سیستم را از خود ویندوز می‌گیریم
# (به‌جای اینکه مقدار ثابت -64 را حدس بزنیم که ممکن است با سیستم شما یکی نباشد)
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        if len(lmList) > 0:
            ...
            # تبدیل صریح به float پایتون (نه numpy.float64) چون COM با
            # نوع‌های numpy مشکل دارد و خطای "parameter is incorrect" می‌دهد
            vol = float(np.interp(length, handRange, [vol_min, vol_max]))
